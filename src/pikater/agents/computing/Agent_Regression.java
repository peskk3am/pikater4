...
public class Agent_Regression extends Agent_ComputingAgent {
	...
	public boolean loadAgent(String agentName) {
		try {
			...
			Instances header = (Instances) v.get(0); // TODO this is not used so
														// far
	...
	protected pikater.ontology.messages.Evaluation evaluateCA() {
		...
		try {
			...
			/*
			 * eval = new Evaluation(train); eval.evaluateModel(cls, test);
			 * System
			 * .out.println(eval.toSummaryString(getLocalName()+" agent: "+
			 * "\nResults\n=======\n", false));
			 */
		} catch (Exception e) {

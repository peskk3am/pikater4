...
public class Agent_Basic extends Agent_Recommender {
	...
	protected pikater.ontology.messages.Agent chooseBestAgent(Data data){
		...
		log(distanceMatrix(), Verbosity.MINIMAL);
		...
		while (itr.hasNext()) {
			...
			log("    " + next_md.getExternal_name() + " d: " + d_new, 2);
		}

		log("Nearest file: " + m_best.getExternal_name(), 1);
		...
		if (agent != null){
			...
			log("No results in database for file " + m_best.getExternal_name(), 1);
			return null;
		}

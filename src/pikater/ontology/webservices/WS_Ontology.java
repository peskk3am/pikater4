...
public class WS_Ontology extends Ontology {
	...
	private WS_Ontology() {
		...
		try {
			...
			cs.add(OPTION_DESCRIPTION,
					(PrimitiveSchema) getSchema(BasicOntology.STRING));
			cs.add(OPTION_SYNOPIS,
					(PrimitiveSchema) getSchema(BasicOntology.STRING));
			cs.add(OPTION_VALUE,
					(PrimitiveSchema) getSchema(BasicOntology.STRING));

			cs = (ConceptSchema) getSchema(RESULTS);
			cs.add(RESULT_OPTIONS,
					(PrimitiveSchema) getSchema(BasicOntology.STRING));
			cs.add(RESULT_ERROR_RATE,
					(PrimitiveSchema) getSchema(BasicOntology.FLOAT));
			cs.add(RESULT_PCT_INCORRECT,
					(PrimitiveSchema) getSchema(BasicOntology.FLOAT));

			AgentActionSchema as = (AgentActionSchema) getSchema(SET_PROBLEM);
			as.add(AGENT_DESCRIPTIONS,
					(PrimitiveSchema) getSchema(BasicOntology.STRING), 1,
			...
			as.setResult((ConceptSchema) getSchema(RESULTS), 0,
					ObjectSchema.UNLIMITED);
